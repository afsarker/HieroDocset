...
#  - Clip and Version will often receive the same name, but we might want them to be different. For instance, we could hide
#    version information from clips.
#  - When Root and Version information is displayed together in the application, the string "Root > Version" is usually used.
...
def user_clipName(clip):
  print "naming_example.user_clipName invoked"
...
def user_versionName(clip):
  print "naming_example.user_versionName invoked"
...
def user_rootName(clip):
  print "naming_example.user_rootName invoked"
...
def extractNames(clip):  
  ...
  if len(pieces) >= 3:
    ...
    regex = "^\\d+\\.dpx$" 
    matches = re.findall(regex, pfile, re.IGNORECASE)
    matching = matching and len(matches) > 0
    ...
    regex = "^version_\\d+$"
    matches = re.findall(regex, pversion, re.IGNORECASE)
    matching = matching and len(matches) > 0
    
    if matching:
      # Build version/clip name of the form: "shotname_01"
      version = pname + "_v" + pversion[8:]
      # Return (True, clipName, versionName, rootName)
      return (True, version, version, pname)

  # This file does not follow our specific convention (we will want to let the default implementation of Hiero handle it)
