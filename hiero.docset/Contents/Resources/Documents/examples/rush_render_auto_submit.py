...
class RushRenderTask(hiero.core.TaskBase):
  ...
  def sendToRush(self, scriptPath, first, last, jobTitle):
    ...
    self._submitJob = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
  ...
  def parseProgressOutput (self):
    ...
    rushProgress = subprocess.Popen(rushStatus, shell=True, stdout=subprocess.PIPE)
    try:
...
"""
    submitInfo = submitInfo.format(jobTitle, renderCommand, scriptPath, start, end, batch)
